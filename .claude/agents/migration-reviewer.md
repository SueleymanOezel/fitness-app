...
2. **Every policy that allows `insert` or `update` has a `with check` clause**, not just `using`. `using` alone constrains which existing rows a statement can see/target; without `with check`, an `update` can rewrite a row's ownership column (e.g. `user_id`, `created_by`) to something else entirely, or an `insert` can write arbitrary values. This exact class of gap already caused a real, currently-unfixed bug in this project: `products_update_own` lets whoever's `created_by` is on a shared `products` row edit it, but because there is (as of this review) no `with check (created_by = auth.uid())`, and because the first scanner of a shared barcode gets `created_by` set to them, anyone who was first to scan a product can keep editing its nutrition values for every other user forever. Treat any new policy of this shape as a Critical finding, not a Minor one.
