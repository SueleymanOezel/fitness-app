...
**Genau hier weitermachen (Stand 07.09.2026):** Hosting ist erledigt, App heißt jetzt **VitaLoop** (siehe „Hosting" im Tech-Stack oben) — dauerhaft erreichbar unter `https://vitaloop.web.app`. **Phase 6 (Design) ist vollständig abgeschlossen — alle fünf Pläne (Fundament, Training, Ernährung, Körper, Analyse-Seiten) umgesetzt, reviewt, gemerged, live deployed und im Wiki dokumentiert.** Details zu Plan 2d im eigenen Abschnitt weiter unten. **Phase-6-Nachschärfung Plan 1 (Design-Tokens) ist gemerged, deployed und gegen Produktion verifiziert** (PR #45, Merge-Commit `cb6edcb`) — Details im Absatz „Phase-6-Nachschärfung Plan 1 (Design-Tokens)" weiter unten. **Plan 2 (Profil nachziehen): Code auf dem Branch fertig und reviewt, noch nicht gemerged** — Details im Absatz „Phase-6-Nachschärfung Plan 2 (Profil nachziehen)" weiter unten. **Nächster Schritt: Plan 2 mergen + deployen + Re-Verifikation gegen die Live-App**, danach Plan 3 (Wall-of-Options), Phase 7 (Härtung) folgt erst danach — deren Spec/Plan noch nicht geschrieben ist.
...
**Noch offen:** PR erstellen und mergen, danach `npm ci && npm run build && firebase deploy --only hosting:vitaloop` im Hauptcheckout, Hash-Sanity-Check und die Re-Verifikation gegen die echte Live-App (`impeccable detect` auf `/profile`, zusätzlich zur bereits erledigten Komponenten-Verifikation).
