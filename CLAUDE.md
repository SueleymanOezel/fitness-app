...
**Phase-6-Nachschärfung Plan 1 (Design-Tokens) (Branch fertig, reviewt, noch nicht gemerged):** Spec: `docs/superpowers/specs/2026-09-06-phase6-nachschaerfung-design.md`, Plan: `docs/superpowers/plans/2026-09-06-phase6-nachschaerfung-plan1-tokens.md`. WCAG-AA-Kontrast-Fix für `--color-text-muted` und neuer `--color-on-bright`-Token; Hover-/Press-/Focus-visible-Motion auf Buttons, `Chip`, `BottomNav` und der Einblend-Animation des Toasts ergänzt, über einen geteilten `interactiveClass`-Export in `src/lib/ui-classes.ts`. **Stand nach Task 7: 753 Tests grün** (106 Dateien), Lint ohne Fehler und Warnungen, `tsc -b --noEmit` sauber, `npm run build` erfolgreich. Entry-Chunk `dist/assets/index-CUc8i8r5.js` 235,74 kB (75,68 kB gzip), CSS `dist/assets/index-CspAU3h4.css` 18,71 kB (4,44 kB gzip) — kein echter Vergleichswert zur `master`-Baseline (Worktree ohne `.env`, wie bei allen vorherigen Phase-6-Plänen). `docs/domaenenmodell.md` geprüft, keine Änderung nötig (letzte Migration bleibt `0007`). **Code auf dem Branch fertig und reviewt (7 Tasks, alle Reviews clean, ein Fix-Round bei Task 3). Whole-Branch-Review (Opus) fand danach 2 Important (BottomNav-Fokusring unterschritt 3:1 gegen die Nav-Pille; `active:scale` lief trotz `prefers-reduced-motion`) — in einer gebündelten Fix-Welle behoben (Commit `9de1d4e`), Scoped Re-Review bestätigte beide ADDRESSED ohne neue Breakage. Zusätzlich unabhängig von Codex (GPT-5.6-Sol) gegengeprüft: "No actionable regressions or correctness defects were identified in the diff." — deckt sich mit dem Opus-Verdict. Noch nicht gemerged. Re-Verifikation gegen die Live-App (impeccable detect + manuelle Browser-Prüfung) steht nach Merge und Deploy noch aus.**
