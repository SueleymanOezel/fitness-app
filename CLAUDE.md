...
**Genau hier weitermachen (Stand 05.09.2026):** Hosting ist erledigt, App heißt jetzt **VitaLoop** (siehe „Hosting" im Tech-Stack oben) — dauerhaft erreichbar unter `https://vitaloop.web.app`. **Phase 6 (Design), Plan 1 (Fundament) ist vollständig umgesetzt, reviewt und gemerged** (PR #40, Merge-Commit `b891d0b`, alle vier CI-Checks grün). **Plan 2a (Training) ist ebenfalls vollständig umgesetzt, reviewt und gemerged** (PR #41, Merge-Commit `f187a4b`, alle CI-Checks grün) — Worktree und Branch entfernt. **Plan 2b (Ernährung) ist geschrieben und auf `master` committet** (Commit `0b73bf1`, `docs/superpowers/plans/2026-09-05-phase6-plan2b-ernaehrung-plan.md`, 7 Tasks) — **Umsetzung noch nicht begonnen**, kein Worktree angelegt. Der Nutzer wurde gefragt, ob direkt mit `superpowers:subagent-driven-development` losgelegt werden soll oder die Design-Entscheidungen im Plan zuerst gegengeprüft werden sollen (insbesondere: Schreibfehler bleiben in diesem Bereich meist inline statt Toast, siehe Rationale im Plan) — **noch keine Antwort erhalten, hier direkt anknüpfen.** Details im eigenen Abschnitt „Phase 6" weiter unten. Danach folgen 2c (Körper), 2d (Analyse-Seiten), noch keiner geschrieben. Härtung rückt auf Phase 7.
...
## Phase 6 – Design (in Arbeit — Plan 1 und Plan 2a gemerged, Plan 2b geschrieben, Umsetzung ausstehend)
...
## Plan 2b – Ernährungsbereich im neuen Design (geschrieben, Umsetzung noch nicht begonnen)

**Plan:** `docs/superpowers/plans/2026-09-05-phase6-plan2b-ernaehrung-plan.md`, Commit `0b73bf1` auf `master`, 7 Tasks. Dashboard-Tagesübersicht als Karte, die Barcode-/Produktsuche-Kette (`ProductPicker`/`BarcodeScanner`/`ManualProductForm`/`AddEntryFlow`) und der Bearbeiten-Flow (`FoodEntryList`/`FoodEntryEditForm`) sollen die in Plan 1 gebauten Bausteine verwenden. Wie bei Plan 2a: kein separates Plan-Brainstorming nötig, direkt von `superpowers:writing-plans` in den Plan (Design-Entscheidungen standen schon im Phase-6-Design-Spec).

**Wichtigste Design-Entscheidung, vom Controller beim Schreiben getroffen (noch nicht vom Nutzer bestätigt):** anders als im Trainingsbereich (Plan 2a) bleiben hier **die meisten Schreibfehler inline statt Toast** — `AddEntryFlow`, `ManualProductForm` und `FoodEntryEditForm` liegen alle in einem Dialog, der bei einem Fehler absichtlich offen bleibt, und ein Toast hinter einem offenen `<dialog>` ist unsichtbar (die Lehre aus Plan 2as Critical-Fund, siehe „Ein Ding, das beim Weiterbauen gilt" oben). Nur der Löschen-Fehler in `FoodEntryList` (kein Dialog beteiligt) wird ein Toast — exakt wie in Plan 2as `TrainingHistoryDetailPage`. `CalorieGoalEditor.tsx`/`ProfilePage.tsx` bleiben bewusst außerhalb (gehören zu keinem der vier Bereichs-Pläne).

**Selbst-Review vor dem Commit fand und behob einen eigenen Testfehler:** ein neu geschriebener Test hätte `AddEntryFlow`s internes „Abbrechen" (setzt nur die Picker-Eingabe zurück, schließt den Dialog **nicht**) mit dem tatsächlichen Dialog-Schließen verwechselt — korrigiert auf den Dialog-eigenen „Schließen"-Button (`Dialog.tsx`s eingebauter ✕-Button, `aria-label="Schließen"`).

**Genau hier weitermachen:** der Nutzer wurde gefragt, ob direkt mit `superpowers:subagent-driven-development` losgelegt werden soll (in einem neuen Worktree, gleiches Muster wie Plan 2a: `git worktree add`, `npm install`, Baseline-Testlauf, Pre-Flight-Scan, dann Task 1 dispatchen) oder ob die Design-Entscheidungen — vor allem die Inline-statt-Toast-Umkehrung — erst gegengeprüft werden sollen. **Noch keine Antwort erhalten, hier direkt anknüpfen.**

**Aufteilung wie bei Phase 5:** Plan 1 (Fundament) — **fertig und gemerged** —, Plan 2a (Training) — **fertig und gemerged** —, Plan 2b (Ernährung) — **geschrieben, Umsetzung ausstehend** —, danach Plan 2c (Körper), 2d (Analyse-Seiten). **Keiner der beiden verbleibenden Bereichs-Pläne ist bisher geschrieben.**

**Wichtig für den Wiedereinstieg:** Plan 2b existiert nur als Plan-Datei auf `master` (Commit `0b73bf1`), noch kein Code umgesetzt, kein Worktree angelegt. Der offene Punkt ist eine reine Bestätigungsfrage an den Nutzer (siehe „Genau hier weitermachen" oben in diesem Abschnitt) — kein technisches Problem, keine Blockade. Bei Fortsetzung: Antwort einholen, dann Worktree anlegen und `superpowers:subagent-driven-development` starten.
