...
**Noch offen:** Whole-Branch-Review (stärkstes Modell) + Codex-Zweitmeinung, PR erstellen und mergen, danach `npm ci && npm run build && firebase deploy --only hosting:vitaloop` im Hauptcheckout, danach Hash-Sanity-Check und die Re-Verifikation gegen die Live-App (`impeccable detect` auf `/profile` + manuelle Browser-Prüfung des Kontrast-Fundes) — genau wie bei Plan 1 aus dem Worktree heraus nicht möglich (kein echtes `.env`), deshalb hier ehrlich als ausstehend vermerkt statt vorweggenommen.
