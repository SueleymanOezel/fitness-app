...
## Phase 4 – Körperbereich (fertig gebaut, wartet auf PR und Merge)

**Stand:** Alle 11 Tasks umgesetzt und je einzeln reviewt, danach Whole-Branch-Review auf dem stärksten Modell, eine Fix-Welle und ein Scoped Re-Review — alle Findings adressiert, keine neue Breakage. Branch `feat-phase4-koerperbereich`, letzter Code-Commit `119aa4a` (die Fix-Welle), danach nur noch dieser Übergabestand, **noch nicht gepusht**, `master` unberührt. **421 Tests grün**, Lint, `tsc -b --noEmit` und `npm run build` sauber (nur die bekannte Bundle-Größen-Warnung, gehört zu Phase 5).
...
**Nächster Schritt:** PR öffnen und mergen, danach die manuelle Verifikation gegen Produktion — die Liste steht am Ende von `docs/superpowers/plans/2026-08-24-phase4-koerperbereich-plan.md`. Der wichtigste Punkt darin ist Schritt 4: ein zweiter Eintrag mit *früherem* Datum darf `profiles.aktuelles_gewicht` **nicht** verändern.

**Migration `0006` ist noch NICHT auf Produktion** — sie läuft erst beim Merge. Legt den privaten Bucket `body-photos` an, samt Policies je Nutzer und seit der Fix-Welle mit `file_size_limit` (5 MB) und `allowed_mime_types`. Es gibt keine lokale Supabase-Instanz, deshalb ist `0006_body_photos_bucket.test.ts` (Textvergleich je Policy-Block) die einzige Prüfung, die diese Migration je bekommt — beim Ändern mitziehen.

**Wiki:** `docs/domaenenmodell.md` ist nachgezogen und nach `../fitness-app.wiki/Domain-Model.md` gespiegelt, dort aber **noch nicht committet/gepusht** — das passiert wie immer nach dem Merge, zusammen mit `Phase-4-Design-Spec` und `Phase-4-Implementation-Plan`.
