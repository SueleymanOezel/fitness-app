...
**Live-Re-Verifikation nur teilweise möglich, ehrlich dokumentiert statt stillschweigend übersprungen:** `npx impeccable detect https://vitaloop.web.app/profile` lief technisch durch (Exit 0, 0 Findings), ist aber **nicht aussagekräftig** — `/profile` hängt hinter `ProtectedRoute` (`src/components/ProtectedRoute.tsx`), ein anonymer Puppeteer-Aufruf ohne Session wird sofort auf `/login` umgeleitet (`<Navigate to="/login" replace />`), das Tool hat also faktisch die Login-Seite gescannt, nicht `/profile`. Frühere „impeccable detect per Browser-Overlay"-Verifikationen liefen über die Chrome-Erweiterung in einer bereits eingeloggten echten Browser-Session; die war in dieser Session nicht verbunden, und es lagen keine Zugangsdaten für einen authentifizierten CLI-Scan vor — beide bewusst nicht erraten/improvisiert. Ersatzweise gilt die bereits **vor dem Merge** abgeschlossene Komponenten-Verifikation (siehe oben: `mx-auto`-Zentrierung per `getBoundingClientRect` bestätigt, Konsole sauber) plus der Hash-verifizierte Deploy — der jetzt live stehende Code ist byte-identisch zum reviewten und verifizierten Commit. **Falls gewünscht:** ein Nutzer-seitiger Kurzcheck auf `https://vitaloop.web.app/profile` (eingeloggt) wäre der einzige noch fehlende Baustein.

**Phase-6-Nachschärfung Plan 3 (Wall-of-Options) — Code fertig und reviewt, noch nicht gemerged:** Spec: `docs/superpowers/specs/2026-09-06-phase6-nachschaerfung-design.md`, Plan: `docs/superpowers/plans/2026-09-07-phase6-nachschaerfung-plan3-walloptions.md`. Zwei Befunde behoben: `ExercisesPage.tsx` (`/training/exercises`) bekommt eine Muskelgruppen-`Chip`-Filterreihe über der Namenssuche — Daten kommen aus dem bereits geladenen `exercises.muskelgruppen_primaer` (keine neue Abfrage), die verfügbaren Gruppen werden dynamisch aus den geladenen Übungen dedupliziert und alphabetisch (de-Locale) sortiert, kein hartkodiertes Enum. `NutritionEntriesPage.tsx` (`/nutrition/entries`): von den vier gleichrangigen „+ Hinzufügen"-Buttons bekommt nur der erste sichtbare Abschnitt (`index === 0` in der `sections.map`, durch `visibleSections`' feste Slot-Reihenfolge immer Frühstück bei einem Standardprofil) `buttonPrimaryClass`, alle weiteren `buttonSecondaryClass` — die unzugeordnete Gruppe hat ohnehin nie einen Button. 2 Umsetzungs-Tasks, umgesetzt per `superpowers:subagent-driven-development` in isoliertem Worktree/Branch `worktree-phase6-nachschaerfung-plan3-walloptions`, beide Reviews clean, keine Fix-Runde:
...
**Noch offen:** Whole-Branch-Review (stärkstes Modell) + Codex-Zweitmeinung, PR erstellen und mergen, danach `npm ci && npm run build && firebase deploy --only hosting:vitaloop` im Hauptcheckout, Hash-Sanity-Check, und danach — laut Spec, „Reihenfolge und Abhängigkeiten" — ein Re-Run von `/impeccable critique` gegen die Live-App, um den Score-Fortschritt über alle drei Nachschärfungs-Pläne zu dokumentieren. Danach ist die Phase-6-Nachschärfung inhaltlich abgeschlossen; das separate, unveränderte Home-Dashboard-Vorhaben (`docs/superpowers/plans/2026-09-06-home-dashboard-plan.md`) und Phase 7 (Härtung) sind eigene künftige Schritte.
