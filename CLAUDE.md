...
**Stand nach Task 5: 738 Tests grün** (106 Dateien — zwei neue Testdateien aus Task 1, `ChartFrame.test.tsx`/`ZeitraumSwitch.test.tsx`), Lint ohne Fehler und Warnungen, `tsc -b --noEmit` sauber, `npm run build` erfolgreich. Entry-Chunk `dist/assets/index-ZnqfvGHZ.js` 235,61 kB (75,63 kB gzip), CSS `dist/assets/index-Th1NYJg3.css` 16,02 kB (4,02 kB gzip) — kein echter Vergleichswert zur `master`-Baseline (Worktree ohne `.env`, wie bei allen vorherigen Plänen dieser Phase). Keine Migration in diesem Plan, `docs/domaenenmodell.md` unverändert (letzte Migration bleibt `0007`).

**Noch offen:** Whole-Branch-Review (Opus) + Codex/GPT-5.6-Sol-Zweitmeinung, danach Merge, Deploy und Wiki-Sync — diesen Abschnitt nach jedem Schritt nachziehen.
