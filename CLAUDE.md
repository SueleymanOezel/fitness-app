...
Zwei Hilfsmodule sind jetzt zwischen Training und Ernährung geteilt statt verdoppelt: `seitenweiseLaden` liegt seit diesem Plan in `src/lib/analysis/paged-query.ts` (vorher nur in `use-training-analysis.ts`), die Wochenhilfsfunktionen `wochenStart`/`wochenLabel` in `src/lib/analysis/wochen.ts` (vorher privat in `training-charts.ts`). Die Makro-Rechnung (`entryMakro`/`sumMakro`, Energie- statt Gramm-Anteile) liegt jetzt in `src/lib/entry-calories.ts` und wird von `DailySummary` und den Graphen E2/E3 gemeinsam benutzt statt einmal in `DailySummary` und einmal in `nutrition-charts.ts` zu stehen.

**Bundle nach Task 8** (`npm run build`): Entry-Chunk `dist/assets/index-BMdxn9ol.js` 231,71 kB (74,53 kB gzip) — deutlich **kleiner** als der Plan-2a-Referenzwert (984,53 kB / 268,30 kB gzip), nicht größer. Recharts bleibt bestätigt außerhalb des Entry-Chunks (0 Treffer für den String „recharts" in `index-BMdxn9ol.js`; weiterhin in den gemeinsamen `CartesianChart`/`Bar`/`Line`/`Legend`/`ReferenceLine`-Chunks), die fünf neuen Ernährungsgraphen selbst sind winzig (0,72–1,06 kB je Komponente, dazu ein `nutrition-charts`-Sammel-Chunk mit 1,95 kB). **Der Rückgang wurde nicht bis auf den verursachenden Commit zurückverfolgt** — `App.tsx` (Routing/Lazy-Grenzen) ist auf diesem Branch gegenüber `master` unverändert, wahrscheinlichster Kandidat sind Dependabot-Bumps auf `master` seit Plan 2a (u. a. Vite 8.2.1→8.2.2, `@vitejs/plugin-react`, `typescript`), die Rollups Standard-Chunk-Splitting beeinflusst haben könnten. Kein Befund, der diesen Task aufhält, aber ein Hinweis für die Härtungsphase, falls die Zahl später unerwartet wieder wächst.
